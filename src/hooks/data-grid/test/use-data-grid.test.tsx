...
describe("useDataGrid", () => {
  ...
  describe("initialization", () => {
    it("should initialize with default values", () => {
      ...
      expect(result.current.grid.focusedCell).toBeNull();
      expect(result.current.grid.editingCell).toBeNull();
      expect(result.current.grid.rowHeight).toBe("short");
    });
    ...
    it("should initialize with custom rowHeight", () => {
      ...
      expect(result.current.grid.rowHeight).toBe("tall");
    });
    ...
    it("should provide table meta with required callbacks", () => {
      ...
      const meta = result.current.grid.tableMeta;
  ...
  describe("cell focus", () => {
    it("should focus a cell via onCellClick", () => {
      ...
      act(() => {
        result.current.grid.tableMeta.onCellClick?.(0, "name");
      });

      expect(result.current.grid.focusedCell).toEqual({
        rowIndex: 0,
        columnId: "name",
      });
    });
    ...
    it("should update focused cell when clicking different cells", () => {
      ...
      act(() => {
        result.current.grid.tableMeta.onCellClick?.(1, "trick");
      });
  ...
  describe("cell editing", () => {
    it("should start editing on double click", () => {
      ...
      act(() => {
        result.current.grid.tableMeta.onCellDoubleClick?.(0, "name");
      });

      expect(result.current.grid.editingCell).toEqual({
        rowIndex: 0,
        columnId: "name",
      });
    });
    ...
    it("should start editing via onCellEditingStart", () => {
      ...
      act(() => {
        result.current.grid.tableMeta.onCellEditingStart?.(0, "name");
      });
    ...
    it("should stop editing via onCellEditingStop", async () => {
      ...
      expect(result.current.grid.editingCell).not.toBeNull();

      await act(async () => {
        result.current.grid.tableMeta.onCellEditingStop?.();
      });
  ...
  describe("cell selection", () => {
    it("should select cells with getIsCellSelected", () => {
      ...
      expect(result.current.grid.tableMeta.getIsCellSelected?.(0, "name")).toBe(
        false,
      );
    ...
    it("should clear selection via onSelectionClear", () => {
      ...
      act(() => {
        result.current.grid.tableMeta.onCellMouseDown?.(0, "name", {
          button: 0,
      ...
      // Clear selection
      act(() => {
        result.current.grid.tableMeta.onSelectionClear?.();
      });
  ...
  describe("copy/cut/paste", () => {
    it("should copy focused cell to clipboard", async () => {
      ...
      // Copy
      await act(async () => {
        await result.current.grid.tableMeta.onCellsCopy?.();
      });
    ...
    it("should not cut in readOnly mode", async () => {
      ...
      // Try to cut
      await act(async () => {
        await result.current.grid.tableMeta.onCellsCut?.();
      });
    ...
    it("should not paste in readOnly mode", async () => {
      ...
      // Try to paste
      await act(async () => {
        await result.current.grid.tableMeta.onCellsPaste?.();
      });
  ...
  describe("data updates", () => {
    it("should call onDataChange when updating data", () => {
      ...
      act(() => {
        result.current.grid.tableMeta.onDataUpdate?.({
          rowIndex: 0,
          columnId: "name",
          value: "Updated Name",
        });
    ...
    it("should update data correctly when filtering is applied", () => {
      ...
      const filteredRows = result.current.getRowModel().rows;
  ...
  describe("row operations", () => {
    it("should call onRowAdd when adding a row", async () => {
      ...
      expect(result.current.grid.onRowAdd).toBeDefined();

      await act(async () => {
        await result.current.grid.onRowAdd?.();
      });
    ...
    it("should provide onRowsDelete callback when prop is provided", () => {
      ...
      expect(result.current.grid.tableMeta.onRowsDelete).toBeDefined();
    });
    ...
    it("should not provide onRowsDelete callback when prop is not provided", () => {
      ...
      expect(result.current.grid.tableMeta.onRowsDelete).toBeUndefined();
    });
  });
  ...
  describe("search functionality", () => {
    it("should provide search state when enableSearch is true", () => {
      ...
      expect(result.current.grid.searchState).toBeDefined();
      expect(result.current.grid.searchState?.searchOpen).toBe(false);
      expect(result.current.grid.searchState?.searchQuery).toBe("");
      expect(result.current.grid.searchState?.searchMatches).toEqual([]);
    });
    ...
    it("should not provide search state when enableSearch is false", () => {
      ...
      expect(result.current.grid.searchState).toBeUndefined();
    });
    ...
    it("should open search panel", async () => {
      ...
      await act(async () => {
        result.current.grid.searchState?.onSearchOpenChange(true);
        // Allow microtask queue to flush
        await Promise.resolve();
      });

      expect(result.current.grid.searchState?.searchOpen).toBe(true);
    });
    ...
    it("should find matches when searching", () => {
      ...
      act(() => {
        result.current.grid.searchState?.onSearch("Tony");
      });

      expect(
        result.current.grid.searchState?.searchMatches.length,
      ).toBeGreaterThan(0);
    });
    ...
    it("should clear search results when query is empty", () => {
      ...
      // Clear search
      act(() => {
        result.current.grid.searchState?.onSearch("");
      });
  ...
  describe("context menu", () => {
    it("should open context menu on cell right-click", () => {
      ...
      act(() => {
        result.current.grid.tableMeta.onCellContextMenu?.(0, "name", mockEvent);
      });

      expect(result.current.grid.contextMenu.open).toBe(true);
      expect(result.current.grid.contextMenu.x).toBe(100);
      expect(result.current.grid.contextMenu.y).toBe(100);
    });
    ...
    it("should close context menu via onContextMenuOpenChange", async () => {
      ...
      // Close context menu
      await act(async () => {
        result.current.grid.tableMeta.onContextMenuOpenChange?.(false);
        await Promise.resolve();
      });

      expect(result.current.grid.contextMenu.open).toBe(false);
    });
  });
  ...
  describe("virtualization", () => {
    it("should provide virtual items", () => {
      ...
      expect(result.current.grid.virtualItems).toBeDefined();
      expect(Array.isArray(result.current.grid.virtualItems)).toBe(true);
    });
    ...
    it("should provide virtualTotalSize", () => {
      ...
      expect(result.current.grid.virtualTotalSize).toBeDefined();
      expect(typeof result.current.grid.virtualTotalSize).toBe("number");
    });
    ...
    it("should provide measureElement function", () => {
      ...
      expect(result.current.grid.measureElement).toBeDefined();
      expect(typeof result.current.grid.measureElement).toBe("function");
    });
  });
  ...
  describe("column size vars", () => {
    it("should compute column size CSS variables", () => {
      ...
      expect(result.current.grid.columnSizeVars).toBeDefined();
      expect(typeof result.current.grid.columnSizeVars).toBe("object");

      // Should have size vars for each column
      expect(
        result.current.grid.columnSizeVars["--col-name-size"],
      ).toBeDefined();
      expect(
        result.current.grid.columnSizeVars["--col-trick-size"],
      ).toBeDefined();
      expect(
        result.current.grid.columnSizeVars["--col-score-size"],
      ).toBeDefined();
    });
  });
  ...
  describe("refs", () => {
    it("should provide dataGridRef", () => {
      ...
      expect(result.current.grid.dataGridRef).toBeDefined();
      expect(result.current.grid.dataGridRef.current).toBeNull(); // Not mounted
    });
    ...
    it("should provide headerRef", () => {
      ...
      expect(result.current.grid.headerRef).toBeDefined();
    });
    ...
    it("should provide footerRef", () => {
      ...
      expect(result.current.grid.footerRef).toBeDefined();
    });
  });
  ...
  describe("row height", () => {
    it("should change row height via tableMeta", async () => {
      ...
      await act(async () => {
        result.current.grid.tableMeta.onRowHeightChange?.("tall");
        await Promise.resolve();
      });
  ...
  describe("paste dialog", () => {
    it("should have closed paste dialog initially", () => {
      ...
      expect(result.current.grid.pasteDialog.open).toBe(false);
    });
    ...
    it("should close paste dialog via onPasteDialogOpenChange", () => {
      ...
      act(() => {
        result.current.grid.tableMeta.onPasteDialogOpenChange?.(false);
      });
  ...
  describe("advanced paste operations", () => {
    it("should parse numbers correctly when pasting", async () => {
      ...
      // Focus a number cell
      act(() => {
        result.current.grid.tableMeta.onCellClick?.(0, "score");
      });
    ...
    it("should handle paste with clipboardText in pasteDialog state", async () => {
      ...
      await act(async () => {
        ...
        await result.current.grid.tableMeta.onCellsPaste?.(false);
      });
    });
  ...
  describe("selection mechanisms", () => {
    it("should handle Ctrl+Click for multi-selection", () => {
      ...
      act(() => {
        result.current.grid.tableMeta.onCellClick?.(0, "name", mockEvent);
      });
    ...
    it("should handle Shift+Click for range selection", () => {
      ...
      act(() => {
        result.current.grid.tableMeta.onCellClick?.(1, "trick", mockEvent);
      });
    ...
    it("should handle mouse drag selection", () => {
      ...
      act(() => {
        result.current.grid.tableMeta.onCellMouseEnter?.(1, "score");
      });

      // Mouse up to end selection
      act(() => {
        result.current.grid.tableMeta.onCellMouseUp?.();
      });
    ...
    it("should stop selection when document mouseup fires during auto-scroll", () => {
      ...
      (
        result.current.grid
          .dataGridRef as React.MutableRefObject<HTMLDivElement>
      ...
      act(() => {
        result.current.grid.tableMeta.onCellMouseEnter?.(2, "score");
      });

      expect(
        result.current.grid.tableMeta.getIsCellSelected?.(2, "score"),
      ).toBe(false);
    });
    ...
    it("should select column when enableColumnSelection is true", () => {
      ...
      act(() => {
        result.current.grid.tableMeta.onColumnClick?.("name");
      });
      ...
      expect(result.current.grid.tableMeta.getIsCellSelected?.(1, "name")).toBe(
        true,
      );
      expect(result.current.grid.tableMeta.getIsCellSelected?.(2, "name")).toBe(
        true,
      );
    });
    ...
    it("should handle right-click without affecting existing selection", () => {
      ...
      act(() => {
        result.current.grid.tableMeta.onCellClick?.(
          0,
          "name",
          mockRightClickEvent,
        );
  ...
  describe("search navigation", () => {
    it("should navigate to next search match", () => {
      ...
      // Search for "Kickflip"
      act(() => {
        result.current.grid.searchState?.onSearch("Kickflip");
      });

      // Navigate to next match
      act(() => {
        result.current.grid.searchState?.onNavigateToNextMatch();
      });

      expect(result.current.grid.searchState?.matchIndex).toBeDefined();
    });
    ...
    it("should navigate to previous search match", () => {
      ...
      // Then navigate back
      act(() => {
        result.current.grid.searchState?.onNavigateToPrevMatch();
      });

      expect(result.current.grid.searchState?.matchIndex).toBe(0);
    });
    ...
    it("should wrap around when navigating past last match", () => {
      ...
      const matchCount =
        result.current.grid.searchState?.searchMatches.length ?? 0;
    ...
    it("should provide searchMatchesByRow computed value", () => {
      ...
      expect(result.current.grid.searchMatchesByRow).toBeDefined();
    });
    ...
    it("should provide activeSearchMatch", () => {
      ...
      expect(result.current.grid.activeSearchMatch).toBeDefined();
    });
    ...
    it("should update search query", async () => {
      ...
      await act(async () => {
        result.current.grid.searchState?.onSearchQueryChange("test query");
        await Promise.resolve();
      });

      expect(result.current.grid.searchState?.searchQuery).toBe("test query");
    });
    ...
    it("should close search and restore focus to last match", async () => {
      ...
      // Close search
      await act(async () => {
        result.current.grid.searchState?.onSearchOpenChange(false);
        await Promise.resolve();
      });
  ...
  describe("row selection operations", () => {
    it("should select row via onRowSelect", () => {
      ...
      const firstRowId = result.current.getRowModel().rows[0]?.id;
      act(() => {
        result.current.grid.tableMeta.onRowSelect?.(
          firstRowId ?? "1",
          true,
          false,
        );
      ...
      const rowSelection = result.current.getState().rowSelection;
  ...
  describe("auto focus", () => {
    it("should auto focus first cell when autoFocus is true", () => {
      ...
      expect(result.current.grid.focusedCell).toBeDefined();
    });
  ...
  describe("sorting and filtering", () => {
    ...
    it("should call onColumnFiltersChange when filters change", () => {
      ...
      // Set a filter
      act(() => {
        result.current.setColumnFilters([{ id: "name", value: "Tony" }]);
      });
    ...
    it("should maintain sorting state", () => {
      ...
      const sorting = result.current.getState().sorting;
    ...
    it("should maintain filter state", () => {
      ...
      const filters = result.current.getState().columnFilters;
  ...
  describe("file operations", () => {
    it("should provide onFilesUpload when prop is provided", () => {
      ...
      expect(result.current.grid.tableMeta.onFilesUpload).toBeDefined();
    });
    ...
    it("should not provide onFilesUpload when prop is not provided", () => {
      ...
      expect(result.current.grid.tableMeta.onFilesUpload).toBeUndefined();
    });
    ...
    it("should provide onFilesDelete when prop is provided", () => {
      ...
      expect(result.current.grid.tableMeta.onFilesDelete).toBeDefined();
    });
    ...
    it("should not provide onFilesDelete when prop is not provided", () => {
      ...
      expect(result.current.grid.tableMeta.onFilesDelete).toBeUndefined();
    });
  });
  ...
  describe("direction (RTL) support", () => {
    it("should handle RTL direction", () => {
      ...
      expect(result.current.grid.dir).toBeDefined();
    });
  });
  ...
  describe("context menu advanced", () => {
    ...
    it("should keep selection when right-clicking already selected cell", async () => {
      ...
      await act(async () => {
        result.current.grid.tableMeta.onCellMouseEnter?.(1, "trick");
        await Promise.resolve();
      });
  ...
  describe("cell selection map optimization", () => {
    it("should provide cellSelectionMap for performance optimization", () => {
      ...
      expect(result.current.grid.cellSelectionMap).toBeNull();
      ...
      expect(result.current.grid.cellSelectionMap).toBeDefined();
    });
  });
  ...
  describe("table meta getters", () => {
    ...
    it("should reflect readOnly in tableMeta getter", () => {
      ...
      expect(result.current.grid.tableMeta.readOnly).toBe(true);
    });
  });
  ...
  describe("onRowAdd with error handling", () => {
    ...
    it("should not proceed if event is defaultPrevented", async () => {
      ...
      await act(async () => {
        await result.current.grid.onRowAdd?.(mockEvent);
      });
