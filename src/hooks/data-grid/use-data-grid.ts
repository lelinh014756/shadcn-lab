...
import {
  type DataGridState,
  type DataGridStore,
  useDataGridStore as useStore,
} from "@/hooks/data-grid/use-data-grid-store";
...
/**
 * Everything the grid renderer reads, carried on the table instance.
 *
 * Mirrors what `useTableCore` does with `options` / `extra`: one transport, so
 * `<DataGrid table={table} />` needs no other props.
 */
interface DataGridSurface<TData> {
  ...
  onRowAdd?: (event?: React.MouseEvent<HTMLDivElement>) => void;
  adjustLayout: boolean;
  height: number;
  stretchColumns: boolean;
}

type DataGridInstance<TData> = ReturnType<typeof useReactTable<TData>> & {
  grid: DataGridSurface<TData>;
};
...
interface UseDataGridProps<TData>
  ...
  height?: number;
  /** Let the last columns absorb leftover horizontal space. */
  stretchColumns?: boolean;
}
...
function useDataGrid<TData>({
  ...
  const grid = React.useMemo<DataGridSurface<TData>>(
    () => ({
      dataGridRef,
      headerRef,
      rowMapRef,
  ...
  // Everything the renderer needs rides on the instance, so `<DataGrid>` takes
  // a single `table` prop instead of a spread of twenty loose values.
  const instance = table as DataGridInstance<TData>;
  instance.grid = grid;
