...
def load_yaml(filename):
    ...
    # skills_data canonical source moved under an-help skill.
    # SCRIPT_DIR is .claude/scripts/, parent is .claude/
    if filename == 'skills_data.yaml':
        canonical = SCRIPT_DIR.parent / 'skills' / 'an-help' / 'scripts' / 'skills_data.yaml'
        if canonical.exists():
...
def generate_commands_yaml():
    ...
    catalog = {
        'metadata': {
            ...
            'description': 'Auto-generated catalog of all available commands in AgentNet Engineer',
...
def generate_skills_yaml():
    ...
    for category in categories:
        category_labels.setdefault(category, category.replace('-', ' ').title())

    # Generate catalog structure
    catalog = {
        'metadata': {
            'title': 'Skills Catalog',
            'description': 'Auto-generated catalog of all available skills in AgentNet Engineer',
            'last_updated': datetime.now().strftime('%Y-%m-%d'),
