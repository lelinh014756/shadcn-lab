...
/**
 * Shorten path for narrow terminals while keeping essential parts
 * Preserves first and last path segments, uses ... for middle
 */
function shortenPath(filePath, maxLength = 25) {
  if (filePath.length <= maxLength) return filePath;
  const parts = filePath.split(path.sep);
  if (parts.length <= 2) return filePath;
  // Keep first segment (~ or drive) and last segment
  const first = parts[0];
  const last = parts[parts.length - 1];
  const available = maxLength - first.length - last.length - 4; // 4 for "..." and separators
  if (available < 0) return last.length > maxLength ? last.slice(0, maxLength - 3) + '...' : last;
  return `${first}/.../${last}`;
}
...
function renderSessionLines(ctx) {
  ...
  // NOTE: Branch first, then directory
  let branchPart = '';
  if (ctx.gitBranch) {
  ...
  let dirPart = `📁 ${ctx.currentDir}`;
  // Shorten path for very narrow terminals
  if (termWidth < 80) {
    const maxPathLen = Math.floor((termWidth - 20) / 2); // Reserve space for icons and other content
    if (ctx.currentDir.length > maxPathLen) {
      dirPart = `📁 ${shortenPath(ctx.currentDir, maxPathLen)}`;
    }
  }
  ...
  // Combined location (branch + dir + plan) - branch first now
  let locationPart = branchPart ? `${branchPart}  ${dirPart}` : dirPart;
  if (planPart) locationPart += `  ${planPart}`;

  // Build session part: 🤖 model  🧠 51%(58k / 200k)  ⌛ time left (usage%)
  let sessionPart = `🤖 ${ctx.modelName}`;

  // Add brain context display: 🧠 0%(0 / 200k) when new session
  if (ctx.contextSize > 0) {
    const formatK = (num) => num >= 1000 ? `${Math.round(num / 1000)}k` : `${num}`;
    sessionPart += `  🧠 ${ctx.contextPercent}%(${formatK(ctx.totalTokens)} / ${formatK(ctx.contextSize)})`;
  }
  ...
  if (visibleLength(allOneLine) <= threshold && statsLen > 0) {
    ...
    // Narrow: session | branch | dir | stats (each on own line, branch before dir)
    lines.push(sessionPart);
 ...
 * Format: "🤖 opus 4.5  🧠 51%(58k / 200k)  ⏰ 2h 16m (38%)  🌿 branch  📁 ~/path"
 */
function renderMinimal(ctx) {
  ...
  if (ctx.contextSize > 0) {
    const formatK = (num) => num >= 1000 ? `${Math.round(num / 1000)}k` : `${num}`;
    parts.push(`🧠 ${ctx.contextPercent}%(${formatK(ctx.totalTokens)} / ${formatK(ctx.contextSize)})`);
  }
...
function renderCompact(ctx) {
  // Line 1: Session info (model + brain context + usage)
  let line1 = `🤖 ${ctx.modelName}`;

  // Add brain context display: 🧠 0%(0 / 200k) when new session
  if (ctx.contextSize > 0) {
    const formatK = (num) => num >= 1000 ? `${Math.round(num / 1000)}k` : `${num}`;
    line1 += `  🧠 ${ctx.contextPercent}%(${formatK(ctx.totalTokens)} / ${formatK(ctx.contextSize)})`;
  }
  ...
  // Line 2: Location (branch + directory) - branch first
  let line2 = '';
  if (ctx.gitBranch) line2 = `🌿 ${ctx.gitBranch}`;
  line2 += line2 ? `  📁 ${ctx.currentDir}` : `📁 ${ctx.currentDir}`;
