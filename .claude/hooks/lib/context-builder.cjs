...
function buildSessionSection(staticEnv = {}) {
  return [
    ...
    `- For subagents: include only the environment details that materially affect execution.`,
    ``
  ];
}
...
function buildRulesSection({ devRulesPath, catalogScript, skillsVenv, plansPath, docsPath }) {
  ...
  if (catalogScript) {
    lines.push(`- Activate skills only on-demand when they materially change task execution. Do not run the skills catalog for every task; use \`python ${catalogScript} --skills\` only for skill discovery or workflow uncertainty.`);
  }
...
function buildModularizationSection() {
  return [
    ...
    `- Reuse existing modules first; modularize only at real logical boundaries. Skip markdown, plain text, scripts, and config files.`,
    ``
  ];
}
