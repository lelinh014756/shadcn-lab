...
Visual verification of frontend implementations via Chrome MCP (Claude Chrome Extension) or `an:chrome-devtools` skill fallback. Detect if frontend-related → check Chrome MCP availability → screenshot + console error check → report. Skip if not frontend.
...
- **Database:** `sqlcmd` / SSMS for SQL Server queries and diagnostics
...
- **Scouting:** Glob/Grep tools or `scout` skill for finding relevant files
