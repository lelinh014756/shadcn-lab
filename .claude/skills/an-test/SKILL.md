...
Browser-based visual testing via `an:chrome-devtools` skill. Screenshots, responsive checks, accessibility audits, form automation, console error collection. Includes auth injection for protected routes.
...
Code tests     → test-execution-workflow.md
  dotnet test / npm test / pytest / go test / cargo test / flutter test
  Coverage: dotnet test --collect:"XPlat Code Coverage" / npm run test:coverage / pytest --cov
...
6. For frontend: run UI tests via `an:chrome-devtools` skill
7. Produce structured summary report

## Tools Integration

- **Test runners**: dotnet test (xUnit/NUnit/MSTest), Jest, Vitest, Mocha, pytest, go test, cargo test, flutter test
- **Coverage**: dotnet test --collect:"XPlat Code Coverage", Istanbul/c8/nyc, pytest-cov, go cover
- **Browser**: `an:chrome-devtools` skill for UI testing (screenshots, ARIA, console, network)
- **Analysis**: `an:ai-multimodal` skill for screenshot analysis
- **Debugging**: `an:debug` skill when tests reveal bugs requiring investigation
- **Thinking**: `an:sequential-thinking` skill for complex test failure analysis
