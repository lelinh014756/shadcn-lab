...
### View Mode
- `/an-preview <file.md>` - View markdown file in novel-reader UI
- `/an-preview <directory/>` - Browse directory contents
- `/an-preview --stop` - Stop running server

### Generation Mode (Markdown)
- `/an-preview --explain <topic>` - Generate visual explanation (ASCII + Mermaid + prose)
- `/an-preview --slides <topic>` - Generate presentation slides (one concept per slide)
- `/an-preview --diagram <topic>` - Generate focused diagram (ASCII + Mermaid)
- `/an-preview --ascii <topic>` - Generate ASCII-only diagram (terminal-friendly)

### Generation Mode (HTML)
- `/an-preview --html --explain <topic>` - Self-contained HTML explanation
- `/an-preview --html --slides <topic>` - Magazine-quality HTML slide deck
- `/an-preview --html --diagram <topic>` - HTML diagram with zoom controls
- `/an-preview --html --diff [ref]` - Visual diff review
- `/an-preview --html --plan-review [plan-file]` - Plan vs codebase comparison
- `/an-preview --html --recap [timeframe]` - Project context snapshot
...
| Flag without topic | Ask user: "Please provide a topic: `/an-preview --explain <topic>`" |
...
| Server startup failure | Check if port in use, try `/an-preview --stop` first |
...
Use `/an-mermaidjs-v11` skill for Mermaid syntax validation.
...
- For `--slides`: consider invoking `/an-ui-ux-pro-max` for richer style selection
