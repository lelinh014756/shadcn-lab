...
# an-scenario — Edge Case & Scenario Explorer
...
| Next Step                          | Skill        | How                                            |
| ---------------------------------- | ------------ | ---------------------------------------------- |
| Generate test cases from scenarios | `/an-test`    | Pass scenario table as input context           |
| Inform implementation plan risks   | `/an-plan`    | Paste Critical/High rows into risk assessment  |
| Deep persona debate on top risks   | `/an-predict` | Feed Critical scenarios as the change proposal |
...
```
/an-scenario src/api/payment.ts
/an-scenario "User registration with OAuth providers"
/an-scenario src/middleware/auth.ts
/an-scenario "Add multi-tenancy to the database layer"
```
