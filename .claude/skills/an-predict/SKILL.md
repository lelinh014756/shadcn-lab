...
# an-predict — Multi-Persona Pre-Analysis
...
- Trivial or low-risk changes (use `/an-debug` for bugs, `/an-plan` for already-decided tasks)
...
| -------------------------------- | ------------- | ------------------------------------------------ |
| Deepen risk scenarios            | `/an-scenario` | Feed Risk Summary rows as feature description    |
| Create implementation plan       | `/an-plan`     | Attach Recommendations as constraints to planner |
...
```
/an-predict "Add WebSocket support for real-time notifications"
/an-predict "Migrate authentication from JWT to session cookies"
/an-predict "Add multi-tenancy to the database layer"
/an-predict "Replace REST API with GraphQL" --files src/api/**/*.ts
```
