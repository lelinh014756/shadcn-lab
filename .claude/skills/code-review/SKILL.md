...
**File Read Policy:** Không đọc lại file chưa thay đổi. Đã đọc trong session → reuse context. Cần 1 section → Grep/Read offset. Delegate subagent → pass content trong prompt.
...
1. Invoke `/an-scout` with edge-case-focused prompt
...
`an-verify` is the canonical source for completion verification in this workflow.
...
**Canonical source:** `../an-verify/references/verification-gate.md`
**Compatibility reference:** `references/verification-before-completion.md`
...
| ----------------------------------- | ---------------------------------------- | ------------------------------------------- |
| `/an-code-review codebase`          | `references/codebase-scan-workflow.md`   | Scan & analyze the codebase                 |
| `/an-code-review codebase parallel` | `references/parallel-review-workflow.md` | Ultrathink edge cases, then parallel verify |
