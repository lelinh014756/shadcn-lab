---
name: cook
description: "End-to-end feature implementation với automatic workflow detection. Use when: user requests new feature, executes a plan file (plan.md/phase-*.md), or says 'implement X', 'làm feature Y', 'cook plan Z', 'triển khai theo plan'. Skip for: bug fixes (→ an-fix), explanation questions, design brainstorming (→ brainstorm), git operations (→ an-git)."
version: 2.3.0
...
**File Read Policy:** Không đọc lại file chưa thay đổi. Đã đọc trong session → reuse context. Delegate subagent → pass content trong prompt, không bắt tự đọc lại.
...
- **Verification:** `an-verify` is the canonical final gate before any `phase complete`, `implementation done`, or finalize claim. `--no-test` does not waive minimum evidence requirements.
...
| Scout | `an:scout` | Optional in code — use repomix-first (see below) |
...
**Codebase navigation (repomix-first — applies to scout phase):**
- `grep_repomix_output("pattern")` → `read_repomix_output(lineRange)` to locate relevant code
