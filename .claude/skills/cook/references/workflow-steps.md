...
- Use `/an-scout ext` or `scout` agent for codebase search
...
- Use `/an-plan --fast` with scout results only
- Minimal planning, focus on action

**Parallel:**

- Use `/an-plan --parallel` for dependency graph + file ownership matrix
...
- Use `AskUserQuestion` to ask: "Validate the plan or approve plan to start implementation?" - "Validate" / "Approve" / "Abort" / "Other" ("Request revisions")
  - "Validate": run `/an-plan validate` slash command
...
- Use `an:ai-multimodal` for image assets
...
Before finalizing, apply `an-verify` as the canonical gate for any `phase complete`, `implementation done`, or equivalent success claim. `--no-test` skips the testing step only; it does not waive minimum verification evidence.
