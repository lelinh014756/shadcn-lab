...
**Templates:** `an:research`, `an:cook`, `an:code-review`, `an:debug`
...
4. Do NOT fall back to subagents. `/an-team` MUST use Agent Teams or abort.
...
## ON `/an-team research <topic>` [--researchers N]:

_Wraps /an-research skill — scope, gather, analyze, report._
...
## ON `/an-team cook <plan-path-or-description>` [--devs N]:

_Wraps /an-cook skill — plan, code, test, review, finalize._
...
6. **DOCS SYNC EVAL** (MANDATORY for cook — from /an-cook finalize):
...
## ON `/an-team review <scope>` [--reviewers N]:

_Wraps /an-code-review skill — scout, review, synthesize with evidence gates._
...
## ON `/an-team debug <issue>` [--debuggers N]:

_Wraps /an-fix skill — root-cause-first, adversarial hypotheses, disprove to converge._
