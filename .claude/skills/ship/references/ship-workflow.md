...
**Important:** `--skip-tests` skips the test step, not the verification gate. `ready to ship` still requires `an-verify` evidence.
...
Before any `ready to ship`, commit-ready, or PR-ready claim:

1. Invoke `an-verify` as the canonical gate.
2. Re-check the strongest available evidence from Steps 4-5.
3. Report `VERIFY_GATE_PASS`, `VERIFY_GATE_FAIL`, or `VERIFY_GATE_BYPASS` honestly.
4. `--skip-tests` does not override this gate.
5. In `ship`, `VERIFY_GATE_BYPASS` is treated as insufficient evidence for readiness unless the workflow explicitly states otherwise.

If the gate fails or is bypassed without an explicit workflow allowance, stop before Step 6.
...
1. Invoke `/an-journal` skill via `journal-writer` subagent in background:
...
1. Invoke `/an-docs update` skill via `docs-manager` subagent in background:
