...
Step 8:  Journal          → Write technical journal via /an-journal
Step 9:  Docs update      → Update project docs via /an-docs update (official only)
...
User says `/an-ship` → run full pipeline → output PR URL.
User says `/an-ship beta` → ship to dev branch with lighter pipeline.
User says `/an-ship official` → ship to main with full docs + journal.
...
- **Verification gate wins.** `--skip-tests` does not authorize `ready to ship`, commit-ready, or PR-ready claims without `an-verify` evidence.
