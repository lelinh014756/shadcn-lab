---
name: an-help
description: "AgentNet usage guide — discover commands, skills, workflows naturally."
argument-hint: "[category|command|task description]"
---

# AgentNet Help

All-in-one AgentNet guide. Run the script and present output based on type markers.
...
```bash
python .claude/skills/an-help/scripts/ck-help.py "$ARGUMENTS"
```

## Output Type Detection

The script outputs a type marker on the first line: `@AN_OUTPUT_TYPE:<type>`

**Read this marker and adjust your presentation accordingly:**

### `@AN_OUTPUT_TYPE:comprehensive-docs`
...
### `@AN_OUTPUT_TYPE:command-details`
...
### `@AN_OUTPUT_TYPE:task-recommendations`
...
- **`/an-plan` → `/an-cook`**: Best for high-risk or complex changes
- **`/an-cook`**: Standalone for straightforward implementation
- **NEVER** claim `/an-plan` is mandatory before `/an-cook`
