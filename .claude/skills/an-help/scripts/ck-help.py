#!/usr/bin/env python3
"""
AgentNet Help - All-in-one guide with dynamic skill discovery.
Scans .claude/skills/ directory to build catalog at runtime.

Usage:
    python an-help.py                    # Overview with quick start
    python an-help.py fix                # Category guide with workflow
    python an-help.py plan validate      # Subcommand details
    python an-help.py debug login error  # Task recommendations
    python an-help.py auth               # Search (unknown word)
"""
...
def emit_output_type(output_type: str) -> None:
    ...
    print(f"@AN_OUTPUT_TYPE:{output_type}")
    print()
...
CATEGORY_GUIDES = {
    ...
    "config": {
        "title": "AgentNet Configuration (.ck.json)",
        "workflow": [
...
def discover_skills(skills_dir: Path) -> dict:
    ...
    for skill_md in sorted(skills_dir.rglob("SKILL.md")):
        ...
        # Skip an-help itself (meta catalog)
        if skill_name == 'an-help':
            continue
...
def show_overview(data: dict, prefix: str) -> None:
    ...
    help_cmd = f"/{prefix}an-help" if prefix else "/an-help"

    print("# AgentNet Skills")
    print()
...
def show_config_guide() -> None:
    ...
    print("# AgentNet Configuration (.ck.json)")
    print()
...
def main():
    ...
    # .claude/skills/an-help/scripts/an-help.py -> .claude/skills
