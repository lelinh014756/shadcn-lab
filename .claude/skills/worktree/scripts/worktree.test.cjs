...
const MONOREPO_DIR = '/home/kai/agentnet';
...
test('create in monorepo with project works', () => {
  ...
  const json = assertJSON(result.output);
  assert(json.wouldCreate.project === 'agentnet-engineer', 'Should detect project');
});
...
test('superproject detection in submodule', () => {
  // Test from agentnet-engineer submodule
  const submodulePath = '/home/kai/agentnet/agentnet-engineer';
...
test('create with partial project match in monorepo', () => {
  ...
  const json = assertJSON(result.output);
  assert(json.wouldCreate.project === 'agentnet-cli', 'Should find agentnet-cli');
});

test('create detects multiple project matches', () => {
  if (!fs.existsSync(MONOREPO_DIR)) return;
  const result = run('create agentnet test-multi --prefix feat --json', { cwd: MONOREPO_DIR });
