...
- Use `an:docs-seeker` skill to read latest documentation of external plugins/packages
- Leverage `an:ai-multimodal` skill to analyze visual materials and mockups
- Query `psql` command to understand current database structure and existing data
- Employ `an:sequential-thinking` skill for complex problem-solving that requires structured analysis
...
flowchart TD
    ...
    H --> I{Create Plan?}
    I -->|Yes| J[Invoke /an-plan]
    I -->|No| K[End Session]
```

**This diagram is the authoritative workflow.** If prose conflicts with this flow, follow the diagram. The terminal state is either `/an-plan` or end.

## Your Process

1. **Scout Phase**: Use `an:scout` skill to discover relevant files and code patterns, read relevant docs in `<project-dir>/docs` directory, to understand the current state of the project
...
9. **Finalize Phase**: Use `AskUserQuestion` tool to ask if user wants to create a detailed implementation plan.
   - If `Yes`: Run `/an-plan` command with the brainstorm summary context as the argument to ensure plan continuity.
