...
4. **Context reminder:** `/an-cook --auto {absolute-plan-path}/plan.md`
...
2. Read codebase docs; if stale/missing: run `/an-scout` to search codebase
...
7. **Context reminder:** `/an-cook {absolute-plan-path}/plan.md`
...
7. **Context reminder:** `/an-cook --parallel {absolute-plan-path}/plan.md`
...
**Invocation:** Use the `Skill` tool with `an-plan` skill and pass the subcommand via args:

```
Skill(skill: "an-plan", args: "red-team {plan-directory-path}")
```
...
| `auto`   | Use the `Skill` tool: `Skill(skill: "an-plan", args: "validate {plan-directory-path}")` |
| `off`    | Skip validation                                                                         |

**Invocation (when prompt mode, user says yes):** Use the `Skill` tool with `an-plan` skill:

```
Skill(skill: "an-plan", args: "validate {plan-directory-path}")
```
...
| Mode     | Cook Command                         |
| -------- | ------------------------------------ |
| fast     | `/an-cook --auto {path}/plan.md`     |
| hard     | `/an-cook {path}/plan.md`            |
| parallel | `/an-cook --parallel {path}/plan.md` |
| two      | `/an-cook {path}/plan.md`            |
