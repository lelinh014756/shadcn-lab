---
name: security-scan
description: "Quick security pass: secrets (grep), dependency audit (npm/pip), common OWASP-style code patterns — read-only report, no auto-fix. For STRIDE + OWASP deep audit and optional iterative fixes, use an-security instead."
argument-hint: "[scope] [--secrets-only] [--deps-only] [--full]"
metadata:
  version: "1.0.0"
---

# Security Scan (quick pass)

Lightweight, fast triage using grep-style checks, package audits, and targeted pattern review. **Does not** run a full STRIDE threat model or apply code changes. No external dependencies required.

## When to use this skill vs `an-security`

| Need | Use |
| ---- | --- |
| Fast secret scan, `npm audit` / `pip audit`, grep for risky patterns, same-day sanity check | **`security-scan`** (this skill) |
| Formal STRIDE + OWASP review, severity-ranked narrative, optional **`--fix`** with guard loops | **`an-security`** |

If the user asks generically for a “security audit” before release, prefer **`an-security`**; if they ask to “scan for secrets”, “quick security check”, or “npm audit”, prefer **`security-scan`**.

## Usage

```
/an-security-scan              # Full scan of current project
/an-security-scan --secrets-only   # Only secret/credential detection
/an-security-scan --deps-only      # Only dependency audit
/an-security-scan src/api/         # Scan specific directory
```
...
This skill handles: Secret detection, dependency auditing, common vulnerability patterns, quick read-only reports.
This skill does NOT handle: STRIDE-style threat modeling, structured compliance narratives, automatic code remediation (use **`an-security`** for audit + optional fix), penetration testing, runtime or infra security.
