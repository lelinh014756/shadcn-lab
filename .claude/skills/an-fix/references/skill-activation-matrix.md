...
| `an:sequential-thinking`      | Step 2 | Structured hypothesis formation — NO guessing |
...
| ------------------------ | ----------------------------------------------------- |
| `an:problem-solving`     | 2+ hypotheses REFUTED in Step 2 diagnosis             |
| `an:sequential-thinking` | Always in Step 2 (mandatory for hypothesis formation) |

## Conditional Activation

| Skill                    | Activate When                                                      |
| ------------------------ | ------------------------------------------------------------------ |
| `an:brainstorm`          | Multiple valid fix approaches, architecture decision (Deep only)   |
| `an:context-engineering` | Fixing AI/LLM/agent code, context window issues                    |
| `an:ai-multimodal`       | UI issues, screenshots provided, visual bugs                       |
| `an:project-management`  | Moderate+ workflows — task hydration, sync-back, progress tracking |
...
| `an:code-review`      | After fix, verify quality and security (Step 5)                        |
...
| -------- | ----------------------------------------------------------------------------------------------------------- |
| Quick    | `scout` (minimal), `an:debug`, `an:sequential-thinking`, `an:code-review`, parallel `Bash` verification     |
| Standard | Above + Tasks, `an:problem-solving` (auto), `an:project-management`, `tester`, parallel `Explore`           |
| Deep     | All above + `an:brainstorm`, `an:context-engineering`, `researcher`, `planner`                              |
| Parallel | Per-issue Task trees + `an:project-management` + `fullstack-developer` agents + coordination via `TaskList` |
...
| Step 2: Diagnose       | Capture pre-fix state → `an:debug` → `an:sequential-thinking` → parallel `Explore` hypotheses → (`an:problem-solving` if 2+ fail) |
...
| ------------------------------- | ------------------------ |
| "AI", "LLM", "agent", "context" | `an:context-engineering` |
| "stuck", "tried everything"     | `an:problem-solving`     |
| "complex", "multi-step"         | `an:sequential-thinking` |
| "which approach", "options"     | `an:brainstorm`          |
| "latest docs", "best practice"  | `researcher` subagent    |
| Screenshot attached             | `an:ai-multimodal`       |
