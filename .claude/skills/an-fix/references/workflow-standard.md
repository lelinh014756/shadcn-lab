...
2. Activate `an:debug` skill. Use `debugger` subagent if needed.
3. Activate `an:sequential-thinking` — form hypotheses through structured reasoning.
...
- Apply `an:problem-solving` skill if stuck
- Use `an:sequential-thinking` for complex logic
...
- Activate `an:project-management` for task sync-back and plan status updates
...
| 2    | `an:debug`, `an:sequential-thinking`, `debugger` subagent, parallel `Explore`, (`an:problem-solving` auto) |
| 3    | `an:problem-solving` (if stuck), `an:sequential-thinking` (complex logic)                                  |
| 4    | `tester` subagent, parallel `Bash` verification                                                            |
| 5    | `code-reviewer` subagent                                                                                   |
| 6    | `an:project-management`, `git-manager`, `docs-manager` subagents                                           |
