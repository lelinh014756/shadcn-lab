...
2. Activate `an:debug` skill (systematic-debugging + root-cause-tracing).
3. Activate `an:sequential-thinking` — structured hypothesis formation.
4. Spawn parallel `Explore` subagents to test each hypothesis.
5. If 2+ hypotheses fail → auto-activate `an:problem-solving`.
...
Implement per plan. Use `an:context-engineering`, `an:sequential-thinking`, `an:problem-solving`.
...
- Activate `an:project-management` for task sync-back, plan status updates, and progress tracking
...
| 2    | `an:debug`, `an:sequential-thinking`, parallel `Explore`, (`an:problem-solving` auto) |
| 3    | `researcher` (runs parallel with steps 1+2)                                           |
| 4    | `an:brainstorm`                                                                       |
| 5    | `planner`                                                                             |
| 6    | `an:problem-solving`, `an:sequential-thinking`, `an:context-engineering`              |
| 7    | `tester`, parallel `Bash` verification                                                |
| 8    | `code-reviewer`                                                                       |
| 9    | `an:project-management`, `docs-manager`, `git-manager`                                |

**Rules:** Don't skip steps. Validate before proceeding. One phase at a time.
**Frontend:** Use `chrome`, `an:chrome-devtools` or any relevant skills/tools to verify.
**Visual Assets:** Use `an:ai-multimodal` for visual assets generation, analysis and verification.
