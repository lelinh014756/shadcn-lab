...
Activate `an:debug` skill. Activate `an:sequential-thinking` for structured analysis.
...
| 2    | `an:debug`, `an:sequential-thinking`  |
| 3    | Parallel `Bash` for verification      |
| 4    | `code-reviewer` subagent              |
| 5    | `git-manager` subagent                |

**Extra:** `an:context-engineering` if dealing with AI/LLM code
