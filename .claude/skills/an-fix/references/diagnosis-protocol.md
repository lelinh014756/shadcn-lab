...
Read, don't assume. Use `an:debug` (systematic-debugging Phase 1).
...
Activate `an:sequential-thinking` skill. Form hypotheses through structured reasoning.
...
Use `an:debug` (root-cause-tracing technique). Trace backward:
...
1. Auto-activate `an:problem-solving` skill
