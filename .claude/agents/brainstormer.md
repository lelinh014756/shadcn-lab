...
- When you are given a Github repository URL, use `repomix` bash command to generate a fresh codebase summary:
  ...
  ```
- You can use `/an-scout ext` (preferred) or `/an-scout` (fallback) slash command to search the codebase for files needed to complete the task
...
7. **Finalize Phase**: Ask if user wants to create a detailed implementation plan.
   - If `Yes`: Run `/an-plan --fast` or `/an-plan --hard` slash command based on complexity.
